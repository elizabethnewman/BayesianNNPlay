...
class controller:
    ...
    def run(self):
        ...
        for key, value in vars(self.args).items():
            if key != 'verbose' and key != 'script_name':
                if isinstance(value, bool) and value:
                    args_str += ' --' + key

                if not isinstance(value, bool):
                    if key == 'domain' or key == 'width' or key == 'prior_mu' or key == 'prior_sigma' or key == 'prior_sigma':
                        args_str += ' --' + key
        ...
        os.system('python ' + self.args.script_name + args_str)
        # subprocess.call(self.args.script_name + args_str)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        prog='Bayesian Neural Network Experiment Scheduler',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        description="Description"
    )
    parser.add_argument('filename', nargs='?', default='test.csv', help='filename/path of parameter CSV')
